...
git commit -m "JellyFusion v2.0.3 - full i18n panels, add-studio, save fallback, logo, about cleanup" | Out-Null
...
git tag -f -a $tag -m "JellyFusion v2.0.3 - all-in-one plugin"
...
try {
    ...
    if ($versions -contains '2.0.3.0') { $ok = $true }
} catch {
...
Write-Host ''
if ($ok) {
    Write-Host '==> SUCCESS - v2.0.3 is LIVE on GitHub' -ForegroundColor Green
    Write-Host ''
    ...
    Write-Host '    https://github.com/KOOL4/JellyFusion/releases/tag/v2.0.3'
} else {
    Write-Warning 'Remote manifest does not yet show v2.0.3.0 - raw cache is still warm.'
