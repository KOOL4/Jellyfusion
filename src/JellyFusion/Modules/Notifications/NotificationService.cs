...
public class NotificationService
{
    ...
    public async Task SendTestAsync(string channel, CancellationToken ct = default)
    {
        ...
        const string title = "✅ JellyFusion — Test notification";
        const string body  = "If you're reading this, notifications are working correctly.";

        switch ((channel ?? "").ToLowerInvariant())
        {
            case "discord":
                if (cfg.Discord.Enabled && !string.IsNullOrEmpty(cfg.Discord.WebhookUrl))
                    await SendDiscordAsync(cfg.Discord.WebhookUrl!, title, body, ct);
                else
